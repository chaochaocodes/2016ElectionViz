...
function adjustCoord(d, axis) {
  ...
  if (abbrevAdjust[d.properties.name]) {
    return center + abbrevAdjust[d.properties.name][key];
  }
  return center;
}
...
d3.json("lib/us-states.json", function(json) {
  let enterSelection = mapSVG.selectAll("path")
                             .data(json.features)
                             .enter()
  ...
  enterSelection.append("svg:text")
                 .text(d => {
                   // if (noAbbrevList[d.properties.name]) return "";
                   return stateAbbrev[d.properties.name];
                 })
